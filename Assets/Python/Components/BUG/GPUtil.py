...
# MOD - START - Realism
g_gpUnitClassTypes = None
g_gpUnitClassIcons = None
#g_gpUnitTypes = None
# MOD - END - Realism
g_gpColors = None
g_unitIcons = None
...
def init():
	...
	"""
	global g_gpUnitClassTypes
	g_gpUnitClassTypes = [None] * NUM_GP
	for i, s in enumerate(g_gpUnitClassList):
		g_gpUnitClassTypes[i] = gc.getInfoTypeForString(s)
	# MOD - END - Realism
	...
	global g_gpUnitClassIcons
	g_gpUnitClassIcons = {}

	g_gpUnitClassIcons[g_gpUnitClassTypes[GP_SPY]] = FontUtil.getChar(FontSymbols.COMMERCE_ESPIONAGE_CHAR)
	g_gpUnitClassIcons[g_gpUnitClassTypes[GP_ENGINEER]] = FontUtil.getChar(FontSymbols.YIELD_PRODUCTION_CHAR)
	g_gpUnitClassIcons[g_gpUnitClassTypes[GP_MERCHANT]] = FontUtil.getChar(FontSymbols.COMMERCE_GOLD_CHAR)
	g_gpUnitClassIcons[g_gpUnitClassTypes[GP_SCIENTIST]] = FontUtil.getChar(FontSymbols.COMMERCE_RESEARCH_CHAR)
	g_gpUnitClassIcons[g_gpUnitClassTypes[GP_ARTIST]] = FontUtil.getChar(FontSymbols.COMMERCE_CULTURE_CHAR)
	g_gpUnitClassIcons[g_gpUnitClassTypes[GP_PROPHET]] = FontUtil.getChar(FontSymbols.RELIGION_CHAR)
	g_gpUnitClassIcons[g_gpUnitClassTypes[GP_GENERAL]] = FontUtil.getChar(FontSymbols.GREAT_GENERAL_CHAR)

	for iUnit in range(gc.getNumUnitInfos()):
		iUnitClass = gc.getUnitInfo(iUnit).getUnitClassType()
		if (iUnitClass in g_gpUnitClassIcons):
			g_unitIcons[iUnit] = g_gpUnitClassIcons[iUnitClass]
	# MOD - END - Realism

def getUnitType(gpType):
	# MOD - START - Realism
	#return g_gpUnitTypes[gpType]
	return gc.getCivilizationInfo(gc.getGame().getActiveCivilizationType()).getCivilizationUnits(g_gpUnitClassTypes[gpType])
	# MOD - END - Realism
...
def findNextCity():
	...
	iThreshold = player.greatPeopleThreshold(False) * 100
	bestCity = None
	for city in PlayerUtil.playerCities(player):
		iRate = city.getTrueGreatPeopleRate()
		if (iRate > 0):
			iProgress = city.getGreatPeopleProgress() * 100
