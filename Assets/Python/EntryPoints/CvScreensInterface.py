...
def randomizeLeaders(argsList):
	iButtonId = argsList[0]
	if iButtonId == 0:
		for iPlayerX in xrange(CyGlobalContext().getMAX_PLAYERS()):
			pPlayerX = CyGlobalContext().getPlayer(iPlayerX)
			if pPlayerX.isAlive() and not pPlayerX.isHuman():
				if CyGlobalContext().getCivilizationInfo(pPlayerX.getCivilizationType()) != -1:
					lLeader = []
					for iLeader in xrange(CyGlobalContext().getNumLeaderHeadInfos()):
						if CyGame().isLeaderEverActive(iLeader): continue
						if iLeader == CyGlobalContext().getDefineINT("BARBARIAN_LEADER"): continue
						if not CyGame().isOption(GameOptionTypes.GAMEOPTION_LEAD_ANY_CIV):
							if not CyGlobalContext().getCivilizationInfo(pPlayerX.getCivilizationType()).isLeaders(iLeader): continue
						lLeader.append(iLeader)
					if len(lLeader) > 0:
						iNewLeader = lLeader[CyGame().getSorenRandNum(len(lLeader), "Chose Leader")]
						if iNewLeader >= 0:
							pPlayerX.changeLeader(iNewLeader)
							pPlayerX.setName(CyGlobalContext().getLeaderHeadInfo(iNewLeader).getDescription())
	return 0
