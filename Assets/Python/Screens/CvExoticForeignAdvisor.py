...
############################################
		...
		if self.H_SCREEN < 720: self.H_SCREEN = 710
		self.Y_SCREEN = self.H_SCREEN / 2
		...
		self.L_SCREEN = 70
		if self.H_SCREEN < 720: self.L_SCREEN = 20
...
## Platy Foreign End ##
		...
		#Horatius' comment: below code passed on into drawlinks (just making clear)
...
##			xLink += self.LABEL_WIDTH_LIST[i]
	...
	def drawActive (self, bInitial):
		...
		iCorrectedWidth = self.W_SCREEN
		if iCorrectedWidth > 1500: iCorrectedWidth = 1500		
		screen.addPanel(mainPanelName, "", "", True, True, (50 + ((self.W_SCREEN - iCorrectedWidth)/2)), 100, iCorrectedWidth - 100, self.H_SCREEN - 200, PanelStyles.PANEL_STYLE_EMPTY)
...
#		ExoticForPrint ("Entered drawInfo")
	...
	def drawInfoImproved (self, bInitial):
		...
		iCorrectedWidth = self.W_SCREEN
		if iCorrectedWidth > 1500: iCorrectedWidth = 1500		
		iLeft = iOutsideGap + ((self.W_SCREEN - iCorrectedWidth)/2)
		iTop = 50 + iOutsideGap
		iWidth = iCorrectedWidth - (2 * iOutsideGap)
...
#				ExoticForPrint ("Player = %d" % iLoopPlayer)
		...
		if self.X_Spread < 26: self.X_Spread = 26
			...
			if self.ltPlayerMet[iLoopPlayer]:
				if (iLoopPlayer != self.iActiveLeader):
					...
					if mCount > 24:					
						...
						iButtonSize = 48					
					screen.addCheckBoxGFCAt(panelName, szName, gc.getLeaderHeadInfo(gc.getPlayer(iLoopPlayer).getLeaderType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), self.X_GLANCE_OFFSET + (self.X_Spread * nCount), self.Y_GLANCE_OFFSET, iButtonSize, iButtonSize, WidgetTypes.WIDGET_LEADERHEAD, iLoopPlayer, self.iActiveLeader, ButtonStyles.BUTTON_STYLE_LABEL, False)
				...
				if self.ltPlayerMet[j]:
					if j != self.iActiveLeader:
						...
						if nAttitude != None:
							...
							szText = ""
						screen.setTextAt (szName, playerPanelName, szText, CvUtil.FONT_CENTER_JUSTIFY, self.X_GLANCE_OFFSET - 2 + (self.X_Spread * nCount), self.Y_GLANCE_OFFSET + self.Y_Text_Offset, -0.1, FontTypes.SMALL_FONT, 
								*BugDll.widgetVersion(2, "WIDGET_LEADERHEAD_RELATIONS", j, iLoopPlayer, WidgetTypes.WIDGET_LEADERHEAD, j, iLoopPlayer))
						nCount += 1
			if nCount > 24:
				screen.attachImageButton(playerPanelName, "", gc.getLeaderHeadInfo(gc.getPlayer(iLoopPlayer).getLeaderType()).getButton(), GenericButtonSizes.BUTTON_SIZE_24, WidgetTypes.WIDGET_LEADERHEAD, iLoopPlayer, self.iActiveLeader, False)			
			elif nCount > 8:
...
## Platy Bonus Screen ##
		...
		screen.addMultiListControlGFC( self.availableMultiList, ""
									 , self.SURPLUS_ICONS_X, self.SURPLUS_ICONS_Y - self.RESOURCE_ICON_SIZE
									 , min(self.SURPLUS_WIDTH, self.RESOURCE_ICON_SIZE * len(listSurplus)), self.RESOURCE_ICON_SIZE *3
									 , 1, 32, 32, TableStyles.TABLE_STYLE_EMPTY )

		self.availableTable = self.getNextWidgetName()
		iMaxIndex = (self.SURPLUS_WIDTH + self.RES_LEFT_RIGHT_SPACE) / self.RESOURCE_ICON_SIZE -1
		# add the circles behind the amounts
		if (self.RES_SHOW_SURPLUS_AMOUNT_ON_TOP):
			for iIndex in range(len(listSurplus)):
				iX = self.SURPLUS_CIRCLE_X_START + iIndex * self.RESOURCE_ICON_SIZE
				iY = self.SURPLUS_CIRCLE_Y - self.RESOURCE_ICON_SIZE
				if iX >= self.SURPLUS_WIDTH + self.RES_LEFT_RIGHT_SPACE:
					iX = self.SURPLUS_CIRCLE_X_START + (iIndex - iMaxIndex) * self.RESOURCE_ICON_SIZE
					iY += self.RESOURCE_ICON_SIZE
		...
		screen.addTableControlGFC( self.availableTable, min(iMaxIndex, len(listSurplus))
							     , self.SURPLUS_TABLE_X, self.SURPLUS_TABLE_Y - self.RESOURCE_ICON_SIZE
							     , len(listSurplus) * self.RESOURCE_ICON_SIZE, self.TABLE_CONTROL_HEIGHT
							     , False, False, 16, 16, TableStyles.TABLE_STYLE_EMPTY )
		self.availableTable2 = self.getNextWidgetName()
		screen.addTableControlGFC( self.availableTable2, min(iMaxIndex, len(listSurplus))
							     , self.SURPLUS_TABLE_X, self.SURPLUS_TABLE_Y
							     , len(listSurplus) * self.RESOURCE_ICON_SIZE, self.TABLE_CONTROL_HEIGHT
							     , False, False, 16, 16, TableStyles.TABLE_STYLE_EMPTY )
		self.availableTable3 = self.getNextWidgetName()
		screen.addTableControlGFC( self.availableTable3, min(iMaxIndex, len(listSurplus))
							     , self.SURPLUS_TABLE_X, self.SURPLUS_TABLE_Y + self.RESOURCE_ICON_SIZE
							     , len(listSurplus) * self.RESOURCE_ICON_SIZE, self.TABLE_CONTROL_HEIGHT
							     , False, False, 16, 16, TableStyles.TABLE_STYLE_EMPTY )
...
#			screen.addCheckBoxGFCAt (self.mainAvailablePanel, "Foo" + str(iIndex), gc.getBonusInfo (listSurplus[iIndex]).getButton(), ArtFileMgr.getInterfaceArtInfo ("BUTTON_HILITE_SQUARE").getPath(), self.X_GLANCE_OFFSET + (self.RESOURCE_ICON_SIZE * iIndex), 10, 32, 32, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, listSurplus[iIndex], -1, ButtonStyles.BUTTON_STYLE_LABEL, False)
			...
			elif iIndex < iMaxIndex * 2:
				screen.setTableColumnHeader( self.availableTable2, iIndex % iMaxIndex, u"", self.RESOURCE_ICON_SIZE )
			else:
				screen.setTableColumnHeader( self.availableTable3, iIndex % iMaxIndex, u"", self.RESOURCE_ICON_SIZE )
			...
			if iIndex < iMaxIndex:
				...
				screen.setTableText( self.availableTable2, iIndex % iMaxIndex, 0, amountStr, "", WidgetTypes.WIDGET_GENERAL, -1, -1, 0 )
			else:
				screen.setTableText( self.availableTable3, iIndex % iMaxIndex, 0, amountStr, "", WidgetTypes.WIDGET_GENERAL, -1, -1, 0 )
## Platy Bonus Screen ##			
...
## Platy Foreign Start ##
		if inputClass.getFunctionName().find("Toggle") > -1:
			self.handlePlatyToggleRelationshipsCB(inputClass.getData())
		elif inputClass.getFunctionName().find("Hide") > -1:
			self.handlePlatyHideLeadersCB(inputClass.getData())
## Platy Foreign End ##

		return 0
## Platy Foreign Start ##
	def handlePlatyToggleRelationshipsCB ( self, argsList ) :
		CvForeignAdvisor.CvForeignAdvisor.clearAllLines(self)
