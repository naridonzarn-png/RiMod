...
class Revolutions:
		...
		while(loopCity):
			...
			if iRevolution >= self.iDangerZone:
				if CyGame().getSorenRandNum(self.iMaxZone, "Revolution") < iRevolution:
					...
					elif loopCity.getNumBuilding(gc.getInfoTypeForString("BUILDING_RIOTS")) == 1 and loopCity.getPopulation > 4:
	...
	def startUprising(self, pCity, iPlayer):
		...
		iOldPlayer = gc.getBARBARIAN_PLAYER()
		iCulture = 0
		iMaxCulture = 0
		iNewCiv = -1
		iNewLeader = -1
				...
				if pPlayerX.isEverAlive():
					...
					if iCulture > iMaxCulture:
						if iCulture > pCityPlot.getCulture(iPlayer):					
							...
							iCanNew = False
							if pPlayerX.isAlive() and gc.getTeam(pPlayerX.getTeam()).isAVassal() and (iCanOld == False):
								iCanFree = True
								iOldPlayer = iPlayerX
								pOldPlayer = gc.getPlayer(iOldPlayer)
								iNewCiv = pOldPlayer.getCivilizationType()
							else:
								iCanOld = True
								iCanFree = False
								iNewPlayer = iPlayerX

		if iCanOld == False and bFreeCivSlots == True:						## 			No suitable rightful owner and enough free player slots
		...
		if iCanOld == True:
		...
		elif iCanOld == False and (iCanNew == True or iCanFree == True) and bFreeCivSlots == True: 		## 			If we can spawn a new civ, time to pick a leader
			lLeader = []
			...
			if iCanFree == True:
				pOldPlayer = gc.getPlayer(iOldPlayer)
				iCiv = pOldPlayer.getCivilizationType()
				CivInfo = gc.getCivilizationInfo(iCiv)
			elif iCanNew == True:
				iCiv = pNewPlayer.getCivilizationType()
				CivInfo = gc.getCivilizationInfo(iCiv)
			...
			if gc.getMAX_PLAYERS() > CyGame().countCivPlayersEverAlive() and (iCanNew == True or iCanFree == True): 
