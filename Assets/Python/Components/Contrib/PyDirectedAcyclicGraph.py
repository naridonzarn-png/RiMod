...
class PyDirectedAcyclicGraph(object):
	...
	def build(self):
		...
		while (len(self.mGraphs) > mGraphIndex):
			...
			element = mGraph.graph.iterkeys().next()
			mGraph.graph[element].x = 0
			map = {}
			map[0] = set([element])
			for iterlimit in range(10):
				for level in range(min(map.keys()), max(map.keys())+1):
					for element in map[level].copy():
						node = mGraph.graph[element]
						if (node.x != level):
							continue
						if (self.getElementType(element) in self.splitOutgoing):
							continue
							...
							if (not mGraph.graph.has_key(u)):
								for i in range(mGraphIndex - 1, -1, -1):
									if (self.mGraphs[i].graph.has_key(u)):
										mGraph.graph[u] = Node()
										mGraph.graph[u].connectsFrom = self.mGraphs[i].graph[u].connectsFrom.copy()
										mGraph.graph[u].connectsTo = self.mGraphs[i].graph[u].connectsTo.copy()
										break
				...
				for level in range (max(map.keys()), min(map.keys()) - 1, -1):
					for element in map[level].copy():
						...
						if (node.x != level):
							continue
						if (self.getElementType(element) in self.splitIncoming):
							continue
