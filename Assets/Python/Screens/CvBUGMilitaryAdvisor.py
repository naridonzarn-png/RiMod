...
#		self.RESOURCE_ICON_SIZE = 34
	...
	def interfaceScreen(self):
		...
		self.X_TEXT = self.W_SCREEN - 399 - ((self.W_SCREEN - 1024)/3)
		self.Y_TEXT = 190
		self.W_TEXT = 380 + ((self.W_SCREEN - 1024)/3)
		self.H_TEXT = self.H_SCREEN - 268
						
		self.X_LEADERS = 20
		self.Y_LEADERS = 80
		self.W_LEADERS = self.W_SCREEN - 39
		self.H_LEADERS = 100

		self.X_MAP = 20
		self.Y_MAP = 220      # 190
		self.W_MAP = self.W_SCREEN - 444 - ((self.W_SCREEN - 1024)/3)
		self.H_MAP_MAX = self.H_SCREEN - 313  # 500
...
#### Deployment Tab ####
	...
	def UL_initMinimap(self, screen):
		...
		if (self.H_MAP > self.H_MAP_MAX):
			self.W_TEXT = 380 + ((self.W_SCREEN - 1024)/3) - (self.H_MAP_MAX * iMap_W) / iMap_H + self.W_MAP
			self.X_TEXT = self.W_SCREEN - 399 - ((self.W_SCREEN - 1024)/3) + (self.H_MAP_MAX * iMap_W) / iMap_H - self.W_MAP
			self.W_MAP = (self.H_MAP_MAX * iMap_W) / iMap_H
			self.H_MAP = self.H_MAP_MAX
			
			if iMap_H > iMap_W:
				iNewMapW = self.H_MAP
				self.W_TEXT += (iNewMapW - self.W_MAP) - 536
				self.X_TEXT += (iNewMapW - self.W_MAP)
				self.W_MAP = iNewMapW
	...
	def UL_refresh(self, bReload, bRedraw):
		...
		if bRedraw:
			...
			if iNumLeaders >= self.LEADER_COLUMNS:
				...
				iMargin = self.LEADER_MARGIN / 2
			else:
				iButtonSize = self.LEADER_BUTTON_SIZE
				iMargin = self.LEADER_MARGIN
	
			iColumns = int(self.W_LEADERS / (iButtonSize + iMargin))
			...
			for iIndex in range(iNumLeaders):
				...
				x = 20 + self.X_LEADERS + iMargin + (iIndex % iColumns) * (iButtonSize + iMargin)
				y = 10 + self.Y_LEADERS + iMargin + (iIndex // iColumns) * (iButtonSize + iMargin)
