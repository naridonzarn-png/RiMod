...
#
# additional work by Gaurav, Progor, Ket, Vovan, Fitchn, LunarMongoose, Cruel
...
class SevoPediaCivilization:
	...
	def __init__(self, main):
		...
		self.H_MAIN_PANE = 220
		self.W_MAIN_PANE = self.H_MAIN_PANE

		# TR - START
		"""
		Realism doesn't use starting techs so we omit them.
		Move the leader into the starting techs spot.
		Move unique buildings into the leaders spot.
		Widen unique units to fill the page width.
		"""
		...
		self.Y_BUILDING = self.Y_LEADER + self.H_LEADER
		self.W_BUILDING = self.W_LEADER
		self.H_BUILDING = 110
		
		self.X_BUD = self.X_MAIN_PANE
		self.Y_BUD = self.Y_BUILDING + self.H_BUILDING
		self.W_BUD = self.top.R_PEDIA_PAGE - self.X_BUD
		self.H_BUD = 110
		
		self.X_UNIT = self.X_MAIN_PANE
		self.Y_UNIT = self.Y_BUD + self.H_BUD
		self.W_UNIT = self.top.R_PEDIA_PAGE - self.X_UNIT
		self.H_UNIT = 110
		
		self.X_UNITS = self.X_MAIN_PANE
		self.Y_UNITS = self.Y_UNIT + self.H_UNIT
		self.W_UNITS = self.top.R_PEDIA_PAGE - self.X_UNIT
		self.H_UNITS = 110
		
		self.X_TEXT = self.X_MAIN_PANE
		self.Y_TEXT = self.Y_UNITS + self.H_UNITS
	...
	def placeBud(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel(panelName, localText.getText("All Distintive Buildings", ()), "", False, True, self.X_BUD, self.Y_BUD, self.W_BUD, self.H_BUD, PanelStyles.PANEL_STYLE_BLUE50)
		screen.attachLabel(panelName, "", "  ")
	...
	def placeBuilding(self):
		...
		for iBuilding in range(gc.getNumBuildingClassInfos()):
			...
			if (iDefaultBuilding > -1 and iUniqueBuilding > -1 and iDefaultBuilding != iUniqueBuilding):
			"""
			if (iUniqueBuilding > -1 and gc.getBuildingInfo(iUniqueBuilding).isUniqueBuilding()):
...
#RI
	def placeUnits(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel(panelName, localText.getText("All Distinctive Units", ()), "", False, True, self.X_UNITS, self.Y_UNITS, self.W_UNITS, self.H_UNITS, PanelStyles.PANEL_STYLE_BLUE50)
		screen.attachLabel(panelName, "", "  ")
...
#RI
	...
	def placeUnit(self):
		...
		for iUnit in range(gc.getNumUnitClassInfos()):
			...
			if (iDefaultUnit > -1 and iUniqueUnit > -1 and iDefaultUnit != iUniqueUnit):
			"""
			if (iUniqueUnit > -1 and gc.getUnitInfo(iUniqueUnit).isUniqueUnit()):
