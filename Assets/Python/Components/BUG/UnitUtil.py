...
def init():
	...
	for eUnit in range(NUM_UNITS):
		...
		"""
		for i in range(unitInfo.getNumPrereqAndBonuses()):
			eBonus = unitInfo.getPrereqAndBonuses(i)
			if eBonus != -1:
				found = True
				strategicBonuses.add(eBonus)
				BugUtil.debug("  requires %s", gc.getBonusInfo(eBonus).getDescription())
		...
		for i in range(unitInfo.getNumPrereqOrBonuses()):
		# MOD - END - Advanced Prerequisites
		...
		for i in range(unitInfo.getNumPrereqAndTechs()):
		# MOD - END - Advanced Prerequisites
...
def canCityBuildUnit(unitInfo, city, eAskingTeam, checkBonuses=True):
	# TODO: Figure out why on Earth this code is duplicated here in Python land...
	# religion
	...
	if eReligion != -1 and not (city and city.isHolyCity(eReligion)):
		return False
	# building
	# MOD - START - Multiple Building Class Unit Requirements
	# Disabled old code
	"""
	...
	if eBuilding != -1:
		...
		if city.getNumBuilding(eBuilding) == 0:
			...
			if eSpecialBuilding == -1 or not gc.getPlayer(city.getOwner()).isSpecialBuildingNotRequired(eSpecialBuilding):
				return False
	"""
	# checking for required (AND) buildings
	for j in range(unitInfo.getNumPrereqAndBuildingClasses()):
		eBuildingClass = unitInfo.getPrereqAndBuildingClasses(j)
		if (eBuildingClass > -1):
			eBuilding = gc.getCivilizationInfo(gc.getActivePlayer().getCivilizationType()).getCivilizationBuildings(eBuildingClass)
			if eAskingTeam != -1 or not city:
				return False
			if city.getNumBuilding(eBuilding) == 0:
				eSpecialBuilding = gc.getBuildingInfo(eBuilding).getSpecialBuildingType()
	...
	# checking for optional (OR) buildings
	nOr = 0
	for j in range(unitInfo.getNumPrereqOrBuildingClasses()):
		if (unitInfo.getPrereqOrBuildingClasses(j) > -1):
			nOr += 1

	bFound = False
	for j in range(unitInfo.getNumPrereqOrBuildingClasses()):
		eBuildingClass = unitInfo.getPrereqOrBuildingClasses(j)
		if (eBuildingClass > -1):
			eBuilding = gc.getCivilizationInfo(gc.getActivePlayer().getCivilizationType()).getCivilizationBuildings(eBuildingClass)
			if eAskingTeam != -1 or not city:
				continue
	...
	if not bFound and (nOr > 0) :
		return False
	# MOD - END - Multiple Building Class Unit Requirements
