...
class Tracker:
	...
	def calculateSpecialists(self, pCity):
		...
		for eYield in range(YieldTypes.NUM_YIELD_TYPES):
			...
			for eSpec in range(gc.getNumSpecialistInfos()):
				iValue += pCity.getSpecialistYield(eSpec, eYield)
	...
	def fillTable(self, screen, table, eYield, eTileType):
		...
		if iModifier != 0:
			# Subtotal
			self.appendTableTotal(screen, table, eYield, iTotal)
			#screen.setTableText(table, "", "", "", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def appendTable(self, screen, table, bTotal, heading, eYield, iValue, bFraction=False):
		...
		if bTotal:
			...
			if bFraction:
				...
				value = u"%+d" % iValue
		if 	eYield == YieldTypes.YIELD_PRODUCTION:
			screen.setTableText(table, HEADING_COLUMN, self.iRow, u"<font=1>%s</font>" % (heading), "", WidgetTypes.WIDGET_PRODUCTION_MOD_HELP, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
			screen.setTableText(table, VALUE_COLUMN, self.iRow, u"<font=1>%s%c</font>" % (value, cYield), "", WidgetTypes.WIDGET_PRODUCTION_MOD_HELP, -1, -1, CvUtil.FONT_RIGHT_JUSTIFY)
		else:
