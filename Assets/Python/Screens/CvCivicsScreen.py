...
class CvCivicsScreen:
	...
	def __init__(self):
		...
		self.HEADINGS_TOP = 50 # was 70
		self.HEADINGS_SPACING = 5
		self.HEADINGS_BOTTOM = 250 # was 280
		self.HELP_TOP = 320 # was 350
		self.HELP_BOTTOM = 650 # was 630
		self.TEXT_MARGIN = 10 # was 15
		self.BUTTON_SIZE = 20 # was 24
		self.BIG_BUTTON_SIZE = 64
		self.BOTTOM_LINE_TOP = 660
		self.BOTTOM_LINE_WIDTH = 1014
		self.BOTTOM_LINE_HEIGHT = 50
	...
	def interfaceScreen (self):
		...
		if self.H_SCREEN > 1200: self.H_SCREEN = 1200
		if self.H_SCREEN < 800: self.H_SCREEN = screen.getYResolution()
		if self.H_SCREEN < 720: self.H_SCREEN = 720

		self.Y_SCREEN = screen.getYResolution() / 2
		self.W_SCREEN = screen.getXResolution() - 140
		self.X_SCREEN = (screen.getXResolution() - 58) / 2

		self.L_SCREEN = 70
		if self.H_SCREEN == 720: self.L_SCREEN = 0
		...
		if self.H_SCREEN == screen.getYResolution():
			self.X_SCREEN = screen.getXResolution() / 2
			self.W_SCREEN = screen.getXResolution()
			self.L_SCREEN = 0
			
		self.HEADINGS_WIDTH = (self.W_SCREEN - 39) / 5
		self.HEADINGS_BOTTOM = self.H_SCREEN - 513 - ((self.H_SCREEN - 768)*2/3) # was 280
		self.HELP_TOP = self.H_SCREEN - 448 - ((self.H_SCREEN - 768)*2/3) # was 350
		self.HELP_BOTTOM = self.H_SCREEN - 108 # was 630
		self.TEXT_MARGIN = (self.HEADINGS_BOTTOM - self.HEADINGS_TOP) / 20 # was 15
		if self.TEXT_MARGIN == 10: 
			self.BUTTON_SIZE = 20 # was 24
		elif self.TEXT_MARGIN < 15:
			self.BUTTON_SIZE = 24
		elif self.TEXT_MARGIN < 20:
			self.BUTTON_SIZE = 32
		elif self.TEXT_MARGIN < 30:
			self.BUTTON_SIZE = 40			
		else:
			self.BUTTON_SIZE = 64
		self.BIG_BUTTON_SIZE = 64
		self.BOTTOM_LINE_TOP = self.H_SCREEN - 108
		self.BOTTOM_LINE_WIDTH = self.W_SCREEN - 10
		self.BOTTOM_LINE_HEIGHT = 60
		
		self.X_EXIT = self.W_SCREEN - 30
		self.Y_EXIT = self.H_SCREEN - 47

		self.X_CANCEL = self.W_SCREEN - 472
		self.Y_CANCEL = self.H_SCREEN - 47
		
		screen.setRenderInterfaceOnly(True);
		screen.showScreen( PopupStates.POPUPSTATE_IMMEDIATE, False)
	
		# Set the background and exit button, and show the screen
		screen.setDimensions(self.L_SCREEN, self.L_SCREEN, self.W_SCREEN, self.H_SCREEN)
		screen.addDDSGFC(self.BACKGROUND_ID, ArtFileMgr.getInterfaceArtInfo("MAINMENU_SLIDESHOW_LOAD").getPath(), 0, 0, self.W_SCREEN, self.H_SCREEN, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.addPanel( "TechTopPanel", u"", u"", True, False, 0, 0, self.W_SCREEN, 55, PanelStyles.PANEL_STYLE_TOPBAR )
		screen.addPanel( "TechBottomPanel", u"", u"", True, False, 0, self.H_SCREEN - 13, self.W_SCREEN, 55, PanelStyles.PANEL_STYLE_BOTTOMBAR )
	...
	def drawAllButtons(self):				
		...
		for i in range(gc.getNumCivicOptionInfos()):
			...
			if self.TEXT_MARGIN < 15:
				fX = self.HEADINGS_SPACING  + (self.HEADINGS_WIDTH + self.HEADINGS_SPACING) * i
			else:
				fX = self.HEADINGS_SPACING + 5 + (self.HEADINGS_WIDTH + self.HEADINGS_SPACING) * i			
			...
			if self.TEXT_MARGIN < 15:
				...
				screen.setLabel("", "Background",  u"<font=4>" + gc.getCivicOptionInfo(i).getDescription().upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, fX + self.HEADINGS_WIDTH/2, self.HEADINGS_TOP + self.TEXT_MARGIN, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
			...
			for j in range(gc.getNumCivicInfos()):
				if (gc.getCivicInfo(j).getCivicOptionType() == i):										
					...
					elif self.TEXT_MARGIN < 25:
						screen.setText(self.getCivicsTextName(j), "", u"<font=3>" + gc.getCivicInfo(j).getDescription() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, fX + self.BUTTON_SIZE + self.TEXT_MARGIN + 5, fY, 0, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
					else:
						screen.setText(self.getCivicsTextName(j), "", u"<font=4>" + gc.getCivicInfo(j).getDescription() + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, fX + self.BUTTON_SIZE + self.TEXT_MARGIN + 5, fY, 0, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
...
## Revolutions ##
		if (not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_NO_REVOLUTIONS)):
			for i in Revolutions.Revolutions().Civics:
				if gc.getCivicInfo(iCivic).getType() == i[0]:
					szHelpText += u"\n%s%+d %s</font>" %(CyTranslator().getText("[ICON_BULLET]", ()), i[1], CyTranslator().getText("TXT_KEY_REVOLUTION_CIVIC_HELP", ()))
## Revolutions ##
		...
		if self.TEXT_MARGIN < 15:
			screen.addMultilineText(szHelpAreaID, u"<font=1>" + szHelpText + "</font>", fX+5, fY, self.HEADINGS_WIDTH-7, self.HELP_BOTTOM - fY-15, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)				
		elif self.TEXT_MARGIN < 25:
			screen.addMultilineText(szHelpAreaID, u"<font=2>" + szHelpText + "</font>", fX+10, fY, self.HEADINGS_WIDTH-17, self.HELP_BOTTOM - fY-15, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)						
		else:
			screen.addMultilineText(szHelpAreaID, u"<font=3>" + szHelpText + "</font>", fX+15, fY, self.HEADINGS_WIDTH-27, self.HELP_BOTTOM - fY-15, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def updateAnarchy(self):
		...
		screen.setLabel("CivicsRevText", "Background", u"<font=3>" + szText + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.BOTTOM_LINE_TOP + 12//2, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
		...
		screen.setLabel("CivicsUpkeepText", "Background", u"<font=3>" + szText + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.BOTTOM_LINE_TOP + self.BOTTOM_LINE_HEIGHT - 3 * 10, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
