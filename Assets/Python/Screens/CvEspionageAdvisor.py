...
class CvEspionageAdvisor:
	...
	def interfaceScreen (self):
		...
		self.H_SCREEN = screen.getYResolution() - 140
		if self.H_SCREEN > screen.getXResolution(): self.H_SCREEN = screen.getXResolution() - 50
		if self.H_SCREEN > 1200: self.H_SCREEN = 1000
		if self.H_SCREEN < 768: self.H_SCREEN = 768
		self.Y_SCREEN = (self.H_SCREEN / 2) + 12
		self.W_SCREEN = screen.getXResolution() - 140
		self.X_SCREEN = (screen.getXResolution() - 24) / 2

		if self.W_SCREEN < 1201:
			self.W_SCREEN = screen.getXResolution()	
		if self.W_SCREEN > 1600:
			self.W_SCREEN = 1600
		...
		# Header...
		screen.setLabel(self.WIDGET_HEADER, "Background", u"<font=4b>" + localText.getText("TXT_KEY_ESPIONAGE_SCREEN", ()).upper() + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, screen.getXResolution() - self.W_SCREEN - 140, self.Y_TITLE, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
	...
	def drawContents(self):
		...
		self.X_LEFT_PANE = 25
		self.Y_LEFT_PANE = 70
		self.W_LEFT_PANE = 420
		self.H_LEFT_PANE = self.H_SCREEN - 148
		...
		self.Y_SCROLL = 90
		self.W_SCROLL = 380
		self.H_SCROLL = self.H_SCREEN - 188
		...
		self.W_TOTAL_PANE = self.W_SCREEN - 474
		...
		self.W_CITY_LIST = 160 + (self.W_TOTAL_PANE - 550) / 3
		...
		self.W_EFFECTS_LIST = self.W_SCREEN - 814 - (self.W_TOTAL_PANE - 550) / 2 + (self.W_TOTAL_PANE - 550) / 12
		...
		self.W_EFFECTS_COSTS_LIST = 60 + (self.W_TOTAL_PANE - 550) / 12
		...
		if (self.iTargetPlayer != -1):
			...
			for iPlayerID in self.aiKnownPlayers:
				...
				if (self.H_SCREEN > 800):
					...
					iX + 5 , iY - 11, 48, 48, WidgetTypes.WIDGET_GENERAL, self.iLeaderImagesID, iPlayerID, ButtonStyles.BUTTON_STYLE_LABEL, False)
				else: 
					screen.addCheckBoxGFCAt(attach, szName, gc.getLeaderHeadInfo(gc.getPlayer(iPlayerID).getLeaderType()).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(),
					iX + 21 , iY - 11, 32, 32, WidgetTypes.WIDGET_GENERAL, self.iLeaderImagesID, iPlayerID, ButtonStyles.BUTTON_STYLE_LABEL, False)					
				...
				screen.attachPanelAt( attach, szName, "", "", true, false, PanelStyles.PANEL_STYLE_MAIN,
					iX + 5, iY-17, self.W_NAME_PANEL, self.H_NAME_PANEL, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				...
				szText = u"<font=1>" + szTempBuffer + "</font>"
				screen.setLabelAt( szName, attach, szText, 0, iX + 55, iY -13, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 );

				szName = "PointsText%d" %(iPlayerID)
				self.aszPointsTexts.append(szName)
				szText = u"<font=1>" + localText.getText("TXT_KEY_ESPIONAGE_NUM_EPS", (pActiveTeam.getEspionagePointsAgainstTeam(iTargetTeam), )) + "</font>"
				screen.setLabelAt( szName, attach, szText, 0, 280, iY - 13, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 );

				szName = "SpendingText%d" %(iPlayerID)
				self.aszSpendingTexts.append(szName)
				szTempBuffer = localText.getText("TXT_KEY_ESPIONAGE_SCREEN_SPENDING_WEIGHT", ()) + ":" + str(pActivePlayer.getEspionageSpendingWeightAgainstTeam(iTargetTeam))
				szText = u"<font=1>" + szTempBuffer + "</font>"
				screen.setLabelAt( szName, attach, szText, 0, 85, iY + 3, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 );

				szName = "AmountText%d" %(iPlayerID)
				self.aszAmountTexts.append(szName)
				if (pActivePlayer.getEspionageSpending(iTargetTeam) > 0):
					szTempBuffer = u"<color=0,255,0,0>%s</color>" %(localText.getText("TXT_KEY_ESPIONAGE_NUM_EPS_PER_TURN", (pActivePlayer.getEspionageSpending(iTargetTeam), )))
					szText = u"<font=1>" + szTempBuffer + "</font>"
				else:
					szTempBuffer = u"<color=192,0,0,0>%s</color>" %(localText.getText("TXT_KEY_ESPIONAGE_NUM_EPS_PER_TURN", (pActivePlayer.getEspionageSpending(iTargetTeam), )))
					szText = u"<font=1>" + szTempBuffer + "</font>"


				screen.setLabelAt( szName, attach, szText, 0, 247, iY + 3, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 );
				...
				iSize = 16
				self.iIncreaseButtonID2 = 557
				screen.setImageButtonAt("IncreaseButton%d" %(iPlayerID), attach, ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_PLUS").getPath(), 53, iY + 7, iSize, iSize, WidgetTypes.WIDGET_GENERAL, self.iIncreaseButtonID2, iPlayerID );
				self.iIncreaseButtonID = 555
				szName += "IncreaseButton%d" %(iPlayerID)
				self.aszIncreaseButtons.append(szName)
				screen.setImageButtonAt( szName, attach, ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_PLUS").getPath(), 68, iY + 7, iSize, iSize, WidgetTypes.WIDGET_GENERAL, self.iIncreaseButtonID, iPlayerID );
				self.iDecreaseButtonID = 556
				szName = "DecreaseButton%d" %(iPlayerID)
				self.aszDecreaseButtons.append(szName)
				screen.setImageButtonAt( szName, attach, ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_MINUS").getPath(), 83, iY + 7, iSize, iSize, WidgetTypes.WIDGET_GENERAL, self.iDecreaseButtonID, iPlayerID );
				self.iDecreaseButtonID3 = 558
				szName += "DecreaseButton%d" %(iPlayerID)
				self.aszDecreaseButtons.append(szName)
				screen.setImageButtonAt( szName, attach, ArtFileMgr.getInterfaceArtInfo("INTERFACE_BUTTONS_MINUS").getPath(), 98, iY + 7, iSize, iSize, WidgetTypes.WIDGET_GENERAL, self.iDecreaseButtonID3, iPlayerID );
	...
	def refreshScreen(self):
		...
		if (self.iTargetPlayer != -1):
			...
			for iPlayerID in self.aiKnownPlayers:
				...
				szName = "SpendingText%d" %(iPlayerID)
				self.aszSpendingTexts.append(szName)
				szText = u"<font=1>" + localText.getText("TXT_KEY_ESPIONAGE_SCREEN_SPENDING_WEIGHT", ()) + ": %d</font>" %(pActivePlayer.getEspionageSpendingWeightAgainstTeam(iTargetTeam))
				screen.deleteWidget(szName)
				screen.setLabelAt( szName, attach, szText, 0, 115, iY + 3, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 );

				szName = "AmountText%d" %(iPlayerID)

				if (pActivePlayer.getEspionageSpending(iTargetTeam) > 0):
					szText = u"<font=1><color=0,255,0,0>%s</color></font>" %(localText.getText("TXT_KEY_ESPIONAGE_NUM_EPS_PER_TURN", (pActivePlayer.getEspionageSpending(iTargetTeam), )))
				else:
					szText = u"<font=1><color=192,0,0,0>%s</color></font>" %(localText.getText("TXT_KEY_ESPIONAGE_NUM_EPS_PER_TURN", (pActivePlayer.getEspionageSpending(iTargetTeam), )))

				screen.deleteWidget(szName)
				screen.setLabelAt( szName, attach, szText, 0, 265, iY + 3, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 );

				szName = "SpendingIcon%d" %(iPlayerID)
				if (pActivePlayer.getEspionageSpendingWeightAgainstTeam(iTargetTeam) > 0):
					szText = u"<font=1>%c</font>" %(gc.getCommerceInfo(CommerceTypes.COMMERCE_ESPIONAGE).getChar())
				else:
					szText = u""
	...
	def handleInput (self, inputClass):
		...
		if (self.iTargetPlayer != -1):
			...
			if (self.iTargetPlayer != -1):
				...
				if (inputClass.getData1() == self.iIncreaseButtonID):
					self.changeEspionageWeight(inputClass.getData2(), pActivePlayer, 1)
				elif (inputClass.getData1() == self.iIncreaseButtonID2):
					self.changeEspionageWeight(inputClass.getData2(), pActivePlayer, 9)
				...
				##### Decrease Button #####
					...
					self.changeEspionageWeight(inputClass.getData2(), pActivePlayer, -1)
				elif inputClass.getData1() == self.iDecreaseButtonID3:
					self.changeEspionageWeight(inputClass.getData2(), pActivePlayer, - 9)		
		...
		return 0
	def changeEspionageWeight(self, iPlayerID, pActivePlayer, iChange):
		iTargetTeam = gc.getPlayer(iPlayerID).getTeam()
		if iChange < 0:
			if pActivePlayer.getEspionageSpendingWeightAgainstTeam(iTargetTeam) == 0:
				return
			iChange = min(iChange, pActivePlayer.getEspionageSpendingWeightAgainstTeam(iTargetTeam))
		...
		if pActivePlayer.getEspionageSpending(iTargetTeam) > 0:
			szText = u"<font=2><color=0,255,0,0>%s</color></font>" %(localText.getText("TXT_KEY_ESPIONAGE_NUM_EPS_PER_TURN", (pActivePlayer.getEspionageSpending(iTargetTeam), )))
		screen.setLabelAt( "AmountText%d" %(iPlayerID), "LeaderContainer%d" % (iPlayerID), szText, 0, 247 + 40, 15 - 1, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 );
		...
		attach = "LeaderContainer%d" % (iPlayerID)
		iY = 15
		screen.setLabelAt( "SpendingIcon%d" %(iPlayerID), attach, szText, 0, 3, iY - 1, self.Z_CONTROLS, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 );			
		CyInterface().setDirty(InterfaceDirtyBits.Espionage_Advisor_DIRTY_BIT, True)
