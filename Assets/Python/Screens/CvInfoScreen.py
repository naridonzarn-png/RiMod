...
#			szProjectDesc = u"<font=3b>" + pProjectInfo.getDescription().upper() + u"</font>"
			...
			screen.addMultilineText(self.szCityNameWidgets[iWidgetLoop], szCityDesc,
				self.X_COL_1_CITIES_DESC + 6, self.Y_ROWS_CITIES[iWidgetLoop] + self.Y_CITIES_DESC_BUFFER, self.W_CITIES_DESC - 6, self.H_CITIES_DESC - 1, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
#			screen.attachMultilineText( szTextPanel, self.szCityNameWidgets[iWidgetLoop], str(self.iCitySizes[iWidgetLoop]) + " - " + self.szCityNames[iWidgetLoop] + "\n" + self.szCityDescs[iWidgetLoop], WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
			...
			iDistance = 150 + (pCity.getPopulation() * 5)
			if (iDistance > 350):
				iDistance = 350
...
#############################################################################################################
	...
	############### Create ListBox for Wonders ###############
			...
			for iWonderLoop in range(iNumWondersBeingBuilt):
				...
				screen.appendListBoxString( self.szWondersListBox, u"<font=1>" + szWonderName + " (" + szWonderBuiltBy + ")" + u"</font>", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY )
...
#			self.aaWondersBeingBuilt_BUG contains the following:
			...
			szWonderName = u"<font=1>" + pWonderInfo.getDescription() + "</font>"
			szTurnYearBuilt = u"<font=1>%c</font>" % gc.getYieldInfo(YieldTypes.YIELD_PRODUCTION).getChar()
			...
			if AdvisorOpt.isWonderListUsePlayerColor():
				szWonderBuiltBy = u"<font=1>" + localText.changeTextColor(szWonderBuiltBy, color) + "</font>"
				szCityName = u"<font=1>" + localText.changeTextColor(szCityName, color) + "</font>"
...
#			self.aaWondersBuilt_BUG contains the following:
			...
			if iTurnYearBuilt == -9999:
				...
				szTurnYearBuilt = u"<font=1>" + BugUtil.getDisplayYear(iTurnYearBuilt) + "</font>"
