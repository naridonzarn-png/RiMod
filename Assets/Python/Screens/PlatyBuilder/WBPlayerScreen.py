...
class WBPlayerScreen:
	...
	def placeScript(self):
		...
		screen.addMultilineText("PlayerScriptDataText", pPlayer.getScriptData().decode("utf-8", "ignore"), iX, iY + 25, iWidth, iHeight, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def placeReligions(self):
		...
		screen.setTableText("WBPlayerReligions", 0, 0, "<font=3>" + sColor + CyTranslator().getText("TXT_KEY_CULTURELEVEL_NONE", ()) + "</font></color>", CyArtFileMgr().getInterfaceArtInfo("INTERFACE_BUTTONS_CANCEL").getPath(), WidgetTypes.WIDGET_HELP_RELIGION, -1, 0, CvUtil.FONT_LEFT_JUSTIFY )
