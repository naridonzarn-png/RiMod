...
## Platy Helpers ##	
	...
	def onLoadGame(self, argsList):
		...
		if not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_NO_AHEAD):		
			...
			AheadOfTime.AheadOfTime().setAhead()
	...
	def onGameStart(self, argsList):
		...
		if (gc.getGame().getGameTurnYear() == gc.getDefineINT("START_YEAR") and not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_ADVANCED_START)):
			for iPlayer in range(gc.getMAX_PLAYERS()):
				...
				if (player.isAlive() and player.isHuman() and gc.getMap().getMapScriptName().endswith("Map Scenario.CivBeyondSwordWBSave")):
					popupInfo = CyPopupInfo()
					popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_CHANGELEADER)
					popupInfo.setText(u"showLeaderChooser")
					popupInfo.addPopup(iPlayer)	
					
				if (player.isHuman() and gc.getMap().getMapScriptName().endswith("Map Scenario.CivBeyondSwordWBSave")):
					popupInfo = CyPopupInfo()
					popupInfo.setButtonPopupType(ButtonPopupTypes.BUTTONPOPUP_PYTHON)
					popupInfo.setText(localText.getText("TXT_KEY_RAND_LEADERS", ()))
					popupInfo.setOnClickedPythonCallback("randomizeLeaders")
					popupInfo.addPythonButton("Randomize", "")
					popupInfo.addPythonButton("Leave preset leaders", "")
					popupInfo.addPopup(iPlayer)
...
## 			print ("Civ " + str(iCiv) + " " + str(gc.getCivilizationInfo(iCiv).getDescription()))
	...
	def onBeginGameTurn(self, argsList):
		...
		# Absinthe: end
		if not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_NO_AHEAD):
			AheadOfTime.AheadOfTime().checkAhead()		
...
#### messages - wonder destroyed end ####		
	...
	## Platy Builder ##
		...
		# If the new and the old owners belong to the same civilization, 75% of the city culture convert instantly to the new owner's 
		pNewOwner = gc.getPlayer(iNewOwner)
		for iPlayerX in xrange(gc.getMAX_PLAYERS()):
			pPlayerX = gc.getPlayer(iPlayerX)
			if pNewOwner.getCivilizationType() == pPlayerX.getCivilizationType():
				iCulture = 75 * pCity.getCulture(iPlayerX) / 100
				pCity.changeCulture(iNewOwner, iCulture, True)
				pCity.changeCulture(iPlayerX, -iCulture, True)
				
				pCityPlot = pCity.plot()
				iPlotCulture = pCityPlot.getCulture(iPlayerX)
				iPlotCultureTransfer = 75 * iPlotCulture / 100
				pCityPlot.changeCulture(iNewOwner, iPlotCultureTransfer, True)
				pCityPlot.changeCulture(iPlayerX, -iPlotCultureTransfer, True)
				for x in range(-1,2):
					for y in range(-1,2):
						if (x != 0) or (y != 0):
							if CyMap().isPlot(pCity.getX() + x, pCity.getY() + y):
								pIterPlot = CyMap().plot(pCity.getX() + x, pCity.getY() + y)
								iIterPlotCulture = pIterPlot.getCulture(iPlayerX)
								iPlotCultureTransfer = 75 * iIterPlotCulture / 100
								pIterPlot.changeCulture(iNewOwner, iPlotCultureTransfer, True)
								pIterPlot.changeCulture(iPlayerX, -iPlotCultureTransfer, True)
				for x in range(-1,2):
					if CyMap().isPlot(pCity.getX() + x, pCity.getY() + 2):
						pIterPlot = CyMap().plot(pCity.getX() + x, pCity.getY() + y)
						iIterPlotCulture = pIterPlot.getCulture(iPlayerX)
						iPlotCultureTransfer = 75 * iIterPlotCulture / 200
						pIterPlot.changeCulture(iNewOwner, iPlotCultureTransfer, True)
						pIterPlot.changeCulture(iPlayerX, -iPlotCultureTransfer, True)			
					if CyMap().isPlot(pCity.getX() + x, pCity.getY() - 2):
						pIterPlot = CyMap().plot(pCity.getX() + x, pCity.getY() + y)
		...
		## Dynamic City Naming ##
		if not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_NO_CITY_NAMING):
			# If this is a randomly spawned barbarian city
			if gc.getPlayer(pCity.getOriginalOwner()).isBarbarian() and gc.getPlayer(iPreviousOwner).isBarbarian() and pCity.getGameTurnFounded() > 0:		
				name = pNewOwner.getNewCityName()
