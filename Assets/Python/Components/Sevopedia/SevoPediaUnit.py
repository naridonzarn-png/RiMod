...
class SevoPediaUnit:
	...
	def placeRequires(self):
		...
		"""
		for j in range(gc.getUnitInfo(self.iUnit).getNumPrereqAndTechs()):
		...
		nAnd = 0
		#for j in range(gc.getNUM_UNIT_PREREQ_AND_BONUSES()):
		for j in range(gc.getUnitInfo(self.iUnit).getNumPrereqAndBonuses()):
		# MOD - END - Advanced Prerequisites
			if (gc.getUnitInfo(self.iUnit).getPrereqAndBonuses(j) > -1):
				nAnd += 1
		...
		for j in range(gc.getUnitInfo(self.iUnit).getNumPrereqAndBonuses()):
		# MOD - END - Advanced Prerequisites
			eBonus = gc.getUnitInfo(self.iUnit).getPrereqAndBonuses(j)
			if (eBonus > -1):
				if (not bFirst):
		...
		# MOD - END - Multiple Bonus Unit Requirements

		nOr = 0
		# MOD - START - Advanced Prerequisites
		#for j in range(gc.getNUM_UNIT_PREREQ_OR_BONUSES()):
		for j in range(gc.getUnitInfo(self.iUnit).getNumPrereqOrBonuses()):
		# MOD - END - Advanced Prerequisites
		...
		bFirst = True
		for j in range(gc.getUnitInfo(self.iUnit).getNumPrereqAndBuildingClasses()):
			eBuildingClass = gc.getUnitInfo(self.iUnit).getPrereqAndBuildingClasses(j)
			if (eBuildingClass > -1):
				if (not bFirst):
					screen.attachLabel(panelName, "", localText.getText("TXT_KEY_AND", ()))
				else:
					bFirst = False
				if self.top.iActivePlayer == -1:
					iBuilding = gc.getBuildingClassInfo(eBuildingClass).getDefaultBuildingIndex()
				else:
					iBuilding = gc.getCivilizationInfo(gc.getActivePlayer().getCivilizationType()).getCivilizationBuildings(eBuildingClass)
		...
		# count the number of OR buildings
		nOr = 0
		for j in range(gc.getUnitInfo(self.iUnit).getNumPrereqOrBuildingClasses()):
			if (gc.getUnitInfo(self.iUnit).getPrereqOrBuildingClasses(j) > -1):
				nOr += 1

		szLeftDelimeter = ""
		szRightDelimeter = ""
		#  Display a bracket if we have more than one OR building and an AND building
		if (not bFirst):
			if (nOr > 1):
				szLeftDelimeter = localText.getText("TXT_KEY_AND", ()) + "( "
				szRightDelimeter = " ) "
		...
		bFirst = True
		for j in range(gc.getUnitInfo(self.iUnit).getNumPrereqOrBuildingClasses()):
			eBuildingClass = gc.getUnitInfo(self.iUnit).getPrereqOrBuildingClasses(j)
			if (eBuildingClass > -1):
				if (not bFirst):
					screen.attachLabel(panelName, "", localText.getText("TXT_KEY_OR", ()))
				else:
