...
class Civ4lerts:
	...
	def __init__(self, eventManager):
		...
		cityEvent.add(CityRevolution(eventManager))
...
class CityRevolution(AbstractCityAlert):
	"""
	Displays an event when a city has a dangerous Separatism value.

	Test: True if the city is in danger..
	"""
	...
	def checkCity(self, cityId, city, iPlayer, player):
		if not gc.getGame().isOption(GameOptionTypes.GAMEOPTION_NO_REVOLUTIONS):
			pRevPlayer = gc.getPlayer(city.getOwner())
			iGlobalModifer = pRevPlayer.getRevModifier()
			cityRevolution = Revolutions.Revolutions().countRevolution(
				city, city.getOwner(), iGlobalModifer)

			if (cityRevolution > 0 and cityRevolution <= 10):
				message = localText.getText(
						"TXT_KEY_CIV4LERTS_ON_CITY_UNREST", (city.getName(), ))
				addMessageAtCity(iPlayer, message, None, city)
			elif (cityRevolution > 10):
				message = localText.getText(
						"TXT_KEY_CIV4LERTS_ON_CITY_REVOLUTION", (city.getName(), ))
				addMessageAtCity(iPlayer, message, None, city)
