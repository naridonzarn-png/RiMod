...
class SevoPediaReligion:
	...
	def __init__(self, main):
		...
		self.Y_BUILDINGS = self.Y_SPECIAL + self.H_SPECIAL +  10
		self.W_BUILDINGS = 350
		self.H_BUILDINGS = 110
		
		self.X_UNITS = self.X_BUILDINGS + self.W_BUILDINGS + 10
		...
		self.Y_TEXT = self.Y_UNITS + self.H_UNITS + 10
	...
	def placeUnits(self):
		...
		screen.addPanel( panelName, "Units", "", false, true, self.X_UNITS, self.Y_UNITS, self.W_UNITS, self.H_UNITS, PanelStyles.PANEL_STYLE_BLUE50 )
	...
	def placeBuildings(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel( panelName, "Buildings", "", false, true, self.X_BUILDINGS, self.Y_BUILDINGS, self.W_BUILDINGS, self.H_BUILDINGS, PanelStyles.PANEL_STYLE_BLUE50 )
		screen.attachLabel(panelName, "", "  ")

		for iLoopBuilding in range(gc.getNumBuildingInfos()):
			if (iLoopBuilding > -1):
				if (gc.getBuildingInfo(iLoopBuilding).getStateReligion() == self.iReligion):
					szButton = gc.getBuildingInfo(iLoopBuilding).getButton()
					screen.attachImageButton(panelName, "", szButton, GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BUILDING, iLoopBuilding, 1, False )
