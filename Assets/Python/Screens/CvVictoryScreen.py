...
# BUG Additions End
	...
	def showVotingScreen(self):
		...
		# Absinthe: I think this is unused, just a leftover code from previous versions
		...
		#		screen.setTableText(szTable, 1, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_BUILT", (szName, )), sBestButton, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
...
# BUG Additions Start
	...
	def showMembersScreen_BUG(self, iRelVote, iUNVote, iActiveVote, iVoteIdx):
		...
		screen.addTableControlGFC(szHeading, 3, self.X_AREA, self.Y_AREA, self.W_AREA, 30, False, False, 24,24, TableStyles.TABLE_STYLE_STANDARD)
		...
		szTable = self.getNextWidgetName()
		screen.addTableControlGFC(szTable, 6, self.X_AREA, self.Y_AREA + 30, self.W_AREA, self.H_AREA-20-30, False, False, 24,24, TableStyles.TABLE_STYLE_STANDARD)
		screen.enableSelect(szTable, False)
		...
		iRow = screen.appendTableRow(szTable)
		iVoteTimer = gc.getGame().getVoteTimer(iActiveVote) + 1 # the +1 is so that when there will be a vote next turn, this reads "1" rather than "0".
		sString = localText.getText("TXT_KEY_BUG_VICTORY_TURNS_NEXT_VOTE", (iVoteTimer,) )
...
# BUG Additions End
		...
		# Total Culture
		ourCulture = activePlayer.getTeam()
		...
		for iLoopVC in xrange(gc.getNumVictoryInfos()):
			...
			if gc.getGame().isVictoryValid(iLoopVC):
				...
				if (victory.isTargetScore() and gc.getGame().getTargetScore() != 0):
					...
					screen.setTableText(szTable, 2, iRow, activePlayer.getTeam().getName() + ":", sLeaderButton, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
						...
						if (gc.getProjectInfo(i).getVictoryThreshold(iLoopVC) > 0):
							if not self.isApolloBuilt():
								...
								if not bApolloShown:
									...
									#screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_PROJECT_APOLLO_PROGRAM", (szNumber, gc.getProjectInfo(i).getTextKey())), gc.getProjectInfo(i).getButton(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
									screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_PROJECT_APOLLO_PROGRAM", ()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
								...
								if self.isApolloBuiltbyTeam(activePlayer.getTeam()):
									...
									szNumber = unicode(gc.getProjectInfo(i).getVictoryMinThreshold(iLoopVC)) + u"-" + unicode(gc.getProjectInfo(i).getVictoryThreshold(iLoopVC))
									screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_BUILDING", (szNumber, gc.getProjectInfo(i).getTextKey())), gc.getProjectInfo(i).getButton(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
...
# BUG Additions End
				...
				if (victory.isDiploVote()):
					for (iVoteBuilding, iUNTeam, bUnknown) in aiVoteBuilding:
						iRow = screen.appendTableRow(szTable)
						screen.setTableText(szTable, 0, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_ELECTION", (gc.getBuildingInfo(iVoteBuilding).getDescription(), )), gc.getBuildingInfo(iVoteBuilding).getButton(), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
						if (iUNTeam != -1):
							if bUnknown:
								szName = localText.getText("TXT_KEY_TOPCIVS_UNKNOWN", ())
								sBestButton = ""
							...
							screen.setTableText(szTable, 2, iRow, localText.getText("TXT_KEY_VICTORY_SCREEN_BUILT", (szName, )), sBestButton, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
						else:
...
# BUG Additions Start
							...
							screen.setTableText(szTable, 3, iRow, sString + u"<font=2>%c</font>" %(gc.getCommerceInfo(CommerceTypes.COMMERCE_CULTURE).getChar()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
# BUG Additions End
...
# BUG Additions Start
							...
							#screen.setTableText(szTable, 5, iRow, sString, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
							screen.setTableText(szTable, 5, iRow, sString + u"<font=2>%c</font>" %(gc.getCommerceInfo(CommerceTypes.COMMERCE_CULTURE).getChar()), "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
# BUG Additions End
