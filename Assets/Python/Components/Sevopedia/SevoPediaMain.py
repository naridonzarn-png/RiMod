...
class SevoPediaMain(CvPediaScreen.CvPediaScreen):
	...
	def __init__(self):
		...
		self.TECH_GRAPH_ID	= "PediaMainTechGraph"
		# MOD - END - Tech Graph
		...
		self.X_TOC = 60
		self.X_INDEX = 200
		self.X_TECH_GRAPH = 290
		self.Y_TECH_GRAPH = 730
		# MOD - END - Tech Graph
		self.X_BACK = 510
		self.Y_BACK = 730
		self.X_NEXT = 645
		...
		self.mapListGenerators = {
			...
			# MOD - START - Aid
			SevoScreenEnums.PEDIA_AID			: self.placeAid,
			# MOD - END - Aid
			# MOD - START - Detriments
			SevoScreenEnums.PEDIA_DETRIMENTS	: self.placeDetriments,
			# MOD - END - Detriments
...
# MOD - END - No Corporations
		...
		self.mapScreenFunctions = {
			...
			# MOD - START - Aid
			SevoScreenEnums.PEDIA_AID			: SevoPediaAid.SevoPediaAid(self),
			# MOD - END - Aid
			# MOD - START - Detriments
			SevoScreenEnums.PEDIA_DETRIMENTS	: SevoPediaDetriment.SevoPediaDetriment(self),
			# MOD - END - Detriments
...
# MOD - END - No Corporations
	...
	def showContents(self, bForce=False, iCategory=SevoScreenEnums.PEDIA_TECHS):
		...
		if not self.isContentsShowing():
			...
			# MOD - START - Tech Graph
			screen.setText(self.TECH_GRAPH_ID, "Background", self.TECH_GRAPH_TEXT, CvUtil.FONT_LEFT_JUSTIFY, self.X_TECH_GRAPH, self.Y_TECH_GRAPH, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_ACTION, gc.getControlInfo(ControlTypes.CONTROL_TECH_CHOOSER).getActionInfoIndex(), -1)
			# MOD - END - Tech Graph
	...
	def setPediaCommonWidgets(self):
		...
		# MOD - START - Tech Graph
		self.TECH_GRAPH_TEXT = u"<font=4>"  + localText.getText("TXT_KEY_PEDIA_SCREEN_TECH_GRAPH",  ()).upper() + u"</font>"
		# MOD - END - Tech Graph
...
# MOD - END - No Corporations
		...
		# MOD - START - Aid
		self.szCategoryAid			= localText.getText("TXT_KEY_PEDIA_CATEGORY_AID", ())
		# MOD - END - Aid
		# MOD - START - Detriments
		self.szCategoryDetriments	= localText.getText("TXT_KEY_PEDIA_CATEGORY_DETRIMENTS", ())
		# MOD - END - Detriments
		...
		self.categoryList = [
			...
			# MOD - START - Aid
			["PROMOTIONS",	self.szCategoryAid],
			# MOD - END - Aid
			# MOD - START - Detriments
			["PROMOTIONS",	self.szCategoryDetriments],
			# MOD - END - Detriments
...
# MOD - END - No Corporations
	...
	def getUnitList(self):
		...
		# TR update by Mexico 07.09.2010 to show only default Great Person in SevoPedia
		numInfos = gc.getNumUnitInfos()

		infoList = []
		prophet = engineer = merchant = artist = scientist = general = spy = 0
		for i in range( numInfos ):
			unitClass = gc.getUnitInfo(i).getUnitClassType()
			next = 1
			if (unitClass == gc.getInfoTypeForString("UNITCLASS_PROPHET")):
				if (not prophet):
					j = gc.getUnitClassInfo(unitClass).getDefaultUnitIndex()
					prophet = 1
					next = 0
			elif (unitClass == gc.getInfoTypeForString("UNITCLASS_ENGINEER")):
				if (not engineer):
					j = gc.getUnitClassInfo(unitClass).getDefaultUnitIndex()
					engineer = 1
					next = 0
			elif (unitClass == gc.getInfoTypeForString("UNITCLASS_SCIENTIST")):
				if (not scientist):
					j = gc.getUnitClassInfo(unitClass).getDefaultUnitIndex()
					scientist = 1
					next = 0
			elif (unitClass == gc.getInfoTypeForString("UNITCLASS_MERCHANT")):
				if (not merchant):
					j = gc.getUnitClassInfo(unitClass).getDefaultUnitIndex()
					merchant = 1
					next = 0
			elif (unitClass == gc.getInfoTypeForString("UNITCLASS_ARTIST")):
				if (not artist):
					j = gc.getUnitClassInfo(unitClass).getDefaultUnitIndex()
					artist = 1
					next = 0
			elif (unitClass == gc.getInfoTypeForString("UNITCLASS_GREAT_GENERAL")):
				if (not general):
					j = gc.getUnitClassInfo(unitClass).getDefaultUnitIndex()
					general = 1
					next = 0
			elif (unitClass == gc.getInfoTypeForString("UNITCLASS_GREAT_SPY")):
				if (not spy):
					j = gc.getUnitClassInfo(unitClass).getDefaultUnitIndex()
					spy = 1
					next = 0
				...
				next = 0
			if (not next):
				desc = gc.getUnitInfo(j).getDescription()
				infoList.append([desc, j])
		infoList.sort()
		return infoList
		# End of update
	...
	# MOD - START - Aid
	def placeAid(self):
		self.list = self.getAidList()
		self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_AID, gc.getAidInfo)
	
	def getAidList(self):
		return self.getSortedList(gc.getNumAidInfos(), gc.getAidInfo)
	# MOD - END - Aid

	# MOD - START - Detriments
	def placeDetriments(self):
		self.list = self.getDetrimentList()
		self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_DETRIMENT, gc.getDetrimentInfo)
	
	def getDetrimentList(self):
		return self.getSortedList(gc.getNumDetrimentInfos(), gc.getDetrimentInfo)
	# MOD - END - Detriments
...
# MOD - END - No Corporations
	...
	def link(self, szLink):
		...
		# MOD - START - Aid
		elif (szLink == "PEDIA_MAIN_AID"):
			return self.pediaJump(SevoScreenEnums.PEDIA_MAIN, SevoScreenEnums.PEDIA_AID, True, True)
		# MOD - END - Aid
		# MOD - START - Detriments
		elif (szLink == "PEDIA_MAIN_DETRIMENT"):
			return self.pediaJump(SevoScreenEnums.PEDIA_MAIN, SevoScreenEnums.PEDIA_DETRIMENTS, True, True)
		# MOD - END - Detriments
		...
		for i in range(gc.getNumAidInfos()):
			if (gc.getAidInfo(i).isMatchForLink(szLink, False)):
				return self.pediaJump(SevoScreenEnums.PEDIA_AID, i, True, True)
		# MOD - END - Aid
		# MOD - START - Detriments
		for i in range(gc.getNumDetrimentInfos()):
			if (gc.getDetrimentInfo(i).isMatchForLink(szLink, False)):
				return self.pediaJump(SevoScreenEnums.PEDIA_DETRIMENTS, i, True, True)
		# MOD - END - Detriments
