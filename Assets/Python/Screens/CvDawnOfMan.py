...
## end HOF MOD
		...
		screen.addLeaderheadGFC("LeaderHead", self.player.getLeaderType(), AttitudeTypes.ATTITUDE_PLEASED,
			self.X_LEADER_ICON + 5, self.Y_LEADER_ICON + 5, self.W_LEADER_ICON - 10, self.H_LEADER_ICON - 10, WidgetTypes.WIDGET_PEDIA_JUMP_TO_LEADER, self.player.getLeaderType(), -1)
		...
		rel_ofset = 0
		iReligion = gc.getLeaderHeadInfo(self.player.getLeaderType()).getFavoriteReligion()
		if (-1 != iReligion):
			szReligionText = u"<link=literal>" + gc.getReligionInfo(iReligion).getDescription() + u"</link>"
			screen.addMultilineText( "Religion Text", localText.getText("TXT_KEY_PEDIA_FAV_RELIGION", ()) + ": " + szReligionText, self.X_STATS_TEXT, self.Y_STATS_TEXT+20, self.W_STATS_TEXT, self.H_STATS_TEXT, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
			rel_ofset = 20
			
		self.Text_BoxText = CyGameTextMgr().parseCivInfos(self.player.getCivilizationType(), True)
		
		#screen.addMultilineText( "HeaderText4", self.Text_BoxText, self.X_STATS_TEXT, self.Y_STATS_TEXT+30+self.H_RELIGION, self.W_STATS_TEXT - (self.iMarginSpace * 3), self.H_STATS_TEXT - (self.iMarginSpace * 4), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
		screen.addMultilineText( "HeaderText4", self.Text_BoxText, self.X_STATS_TEXT, self.Y_STATS_TEXT+30+rel_ofset, self.W_STATS_TEXT - (self.iMarginSpace * 4), self.H_LEADER_ICON - (self.iMarginSpace * 4) - 30 - rel_ofset, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)	
							
		# Fancy icon things
		screen.addDDSGFC( "IconLeft", ArtFileMgr.getCivilizationArtInfo(gc.getCivilizationInfo(self.player.getCivilizationType()).getArtDefineTag()).getButton(), self.X_FANCY_ICON1 , self.Y_FANCY_ICON , self.WH_FANCY_ICON, self.WH_FANCY_ICON, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIV, self.player.getCivilizationType(), -1 )
		screen.addDDSGFC( "IconRight", ArtFileMgr.getCivilizationArtInfo(gc.getCivilizationInfo(self.player.getCivilizationType()).getArtDefineTag()).getButton(), self.X_FANCY_ICON2 , self.Y_FANCY_ICON , self.WH_FANCY_ICON, self.WH_FANCY_ICON, WidgetTypes.WIDGET_PEDIA_JUMP_TO_CIV, self.player.getCivilizationType(), -1 )
...
## Leader Unique DOM Map Specific ##
	...
	def calculateSizesAndPositions(self):
		...
		if self.H_SCREEN > 900:
			self.H_MAIN_PANEL = 600
		else:
		...
		# < Dawn Of Man Start >
		# MOD - START - National Values
		#self.X_LEADER_TITLE_TEXT = (self.X_FANCY_ICON1+self.WH_FANCY_ICON)+((self.X_FANCY_ICON2 - (self.X_FANCY_ICON1+self.WH_FANCY_ICON))/2) - ((self.W_HEADER_PANEL / 3)/2)
		self.X_LEADER_TITLE_TEXT = (self.X_FANCY_ICON1+self.WH_FANCY_ICON)+((self.X_FANCY_ICON2 - (self.X_FANCY_ICON1+self.WH_FANCY_ICON))/2) - ((self.W_HEADER_PANEL / 2)/2)
		# MOD - END - National Values
		# < Dawn Of Man End   >		
		
		self.Y_LEADER_TITLE_TEXT = self.Y_HEADER_PANEL + self.iMarginSpace + 6
		# MOD - START - National Values
		#self.W_LEADER_TITLE_TEXT = self.W_HEADER_PANEL / 3
		self.W_LEADER_TITLE_TEXT = self.W_HEADER_PANEL / 2
		# MOD - END - National Values
		self.H_LEADER_TITLE_TEXT = self.H_HEADER_PANEL / 2
