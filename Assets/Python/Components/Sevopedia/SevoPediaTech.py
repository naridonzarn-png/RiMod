...
class SevoPediaTech(CvPediaScreen.CvPediaScreen):
	...
	def __init__(self, main):
		...
		self.W_TECH_PANE = self.top.W_PEDIA_PAGE / 2 - 5
		...
		self.X_SPECIAL_PANE = self.X_TECH_PANE + self.W_TECH_PANE + 10
		self.Y_SPECIAL_PANE = self.Y_TECH_PANE
		self.W_SPECIAL_PANE = self.top.R_PEDIA_PAGE - self.X_SPECIAL_PANE
		self.H_SPECIAL_PANE = self.H_TECH_PANE

		self.X_PREREQ_PANE = self.X_TECH_PANE
		self.Y_PREREQ_PANE = self.Y_TECH_PANE + self.H_TECH_PANE + 10
		...
		self.X_BUILDING_PANE = self.X_TECH_PANE
		self.Y_BUILDING_PANE = self.Y_PREREQ_PANE + self.H_PREREQ_PANE +10
		self.W_BUILDING_PANE = self.W_PREREQ_PANE
		self.H_BUILDING_PANE = self.H_PREREQ_PANE

		self.H_UNIT_PANE = self.H_PREREQ_PANE
		self.W_UNIT_PANE = self.W_BUILDING_PANE
		self.X_UNIT_PANE = self.X_BUILDING_PANE + self.W_UNIT_PANE + 10
		self.Y_UNIT_PANE = self.Y_BUILDING_PANE

		self.X_QUOTE_PANE = self.X_TECH_PANE
		self.Y_QUOTE_PANE = self.Y_BUILDING_PANE + self.H_BUILDING_PANE + 10
		self.W_QUOTE_PANE = self.top.R_PEDIA_PAGE - self.X_QUOTE_PANE
		self.H_QUOTE_PANE = 240
	...
	def placeSpecial(self):
		...
		screen.addMultilineText(listName, szSpecialText, self.X_SPECIAL_PANE+5, self.Y_SPECIAL_PANE+10, self.W_SPECIAL_PANE-35, self.H_SPECIAL_PANE-10, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
	...
	def placeQuote(self):
		...
		screen.addMultilineText(szQuoteTextWidget, szQuote, self.X_QUOTE_PANE + 15, self.Y_QUOTE_PANE + 40,self.W_QUOTE_PANE - (15 * 2), self.H_QUOTE_PANE - (15 * 2) - 25, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
