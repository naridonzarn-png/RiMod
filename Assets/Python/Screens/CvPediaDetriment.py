...
class CvPediaDetriment(CvPediaPage.CvPediaPage):
	"Civilopedia Screen for Detriments"

	def __init__(self, section):
		super(CvPediaDetriment, self).__init__(section, CivilopediaPageTypes.CIVILOPEDIA_PAGE_DETRIMENT, WidgetTypes.WIDGET_PEDIA_JUMP_TO_DETRIMENT, "Detriment")
	...
	def getData(self):
		return (self.eDetriment, -1)
	
	def setData(self, eDetriment, iPageData2):
		if (self.eDetriment != eDetriment):
			self.clear()
			self.eDetriment = eDetriment
	
	def getInfo(self):
		return gc.getDetrimentInfo(self.eDetriment)
	...
	def draw(self):
		...
		self.drawSpecial(screen, CyGameTextMgr().getDetrimentHelp(self.eDetriment, True)[1:])
	...
	def drawSuppliedBy(self, screen, ePlayer):
		screen.addPanel(self.record(self.SUPPLIED_BY_PANEL_ID), localText.getText("TXT_KEY_DETRIMENT_INFLICTED_BY", ()), "", False, True, self.SUPPLIED_BY_PANEL_X, self.SUPPLIED_BY_PANEL_Y, self.SUPPLIED_BY_PANEL_W, self.SUPPLIED_BY_PANEL_H, PanelStyles.PANEL_STYLE_BLUE50)
		...
		for eLoopUnitClass in range(gc.getNumUnitClassInfos()):
			...
			if (eLoopUnit > -1):
				...
				for i in range(pUnitClassInfo.getNumDetrimentTypes()):
					eLoopDetriment = pUnitClassInfo.getDetrimentTypes(i)
					if eLoopDetriment == self.eDetriment:
						if ePlayer != -1:
		...
		for eLoopImprovement in range(gc.getNumImprovementInfos()):
			if (eLoopImprovement != -1):
				...
				for i in range(pImprovementInfo.getNumDetrimentTypes()):
					eLoopDetriment = pImprovementInfo.getDetrimentTypes(i)
					if eLoopDetriment == self.eDetriment:
