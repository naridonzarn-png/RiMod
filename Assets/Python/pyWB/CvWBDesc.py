...
class CvPlayerDesc:
	...
	## Platy Builder ##
		...
		# MOD - START - Handicap Score Modifier
		f.write("\tHandicapScoreModifier=%d\n" %(gc.getPlayer(idx).getHandicapScoreModifier()))
		# MOD - END - Handicap Score Modifier
		f.write("EndPlayer\n")
	...
	def read(self, f):
		...
		if (parser.findNextTokenValue(f, "BeginPlayer")!=-1):
			while (true):
				...
				# MOD - START - Handicap Score Modifier
				v = parser.findTokenValue(toks, "HandicapScoreModifier")
				if v!=-1:
					self.iHandicapScoreModifier = int(v)
					continue
				# MOD - END - Handicap Score Modifier					
...
class CvSignDesc:
	...
	def write(self, f, sign):
		...
		f.write("\tplotX=%d\n" %(sign.getPlot().getX(),))
		f.write("\tplotY=%d\n" %(sign.getPlot().getY(),))
		if gc.getPlayer(sign.getPlayerType()):
			f.write("\tplayerType=%d, (%s)\n" %(sign.getPlayerType(), gc.getPlayer(sign.getPlayerType()).getName().encode(fileencoding)))
		else:
			f.write("\tplayerType=%d, (%s)\n" %(sign.getPlayerType(), gc.getPlayer(gc.getBARBARIAN_PLAYER()).getName().encode(fileencoding)))
		uCaption = sign.getCaption()
		# f1rpo: Strip non-ascii
...
## Platy Builder ##
	...
	def applyInitialItems(self):
		...
		for iPlayerLoop in xrange(len(self.playersDesc)):
			...
			# MOD - START - Handicap Score Modifier
			pPlayer.setHandicapScoreModifier(pWBPlayer.iHandicapScoreModifier)
			# MOD - END - Handicap Score Modifier			
