...
def getGridSize(argsList):
	...
	grid_sizes = {
		WorldSizeTypes.WORLDSIZE_SPECIAL:	(26,16),
		...
		WorldSizeTypes.WORLDSIZE_GIANT:		(32,20)
	}
...
def generatePlotTypes():
	...
	valleysFrac = CyFractal()
	terrainFracA = CyFractal()
	terrainFracB = CyFractal()
	lakesFrac = CyFractal()
	...
	worldsizes = {
		WorldSizeTypes.WORLDSIZE_SPECIAL:   [4,5,6],
	...
	# Valley density
	valley_grain = 4 # Clustered grain
	valleys = 16
	valleyEdgesA = 20
	valleyEdgesB = 25

	# Terrain A (Valleys and Ridgelines)
	grainA = grain_list[1] # Ridgeline grain
	valley_listA = [12, 16, 20]
	peak_listA = [70, 76, 81]
	hill_listA = [65, 71, 75]
	valleysA = valley_listA[userInputPeaks]
	peaksA = peak_listA[userInputPeaks]
	hillsA = hill_listA[userInputPeaks]

	# Terrain B (Mountain Ranges)
	grainB = grain_list[userInputGrain] # Clustered grain
	peak_listB = [65, 73, 78]
	hill_listB = [45, 52, 55]
	peaksB = peak_listB[userInputPeaks]
	hillsB = hill_listB[userInputPeaks]
	
	# Lake density
	lake_list = [5, 10, 15, 25, 32]
	lake_grains = [5, 4, 3, 1, 2]
	lakes = lake_list[userInputLakes]
	lake_grain = lake_grains[userInputLakes]

	valleysFrac.fracInit(iW, iH, valley_grain, dice, 0, -1, -1)
	terrainFracA.fracInit(iW, iH, grainA, dice, 0, -1, -1)
	terrainFracB.fracInit(iW, iH, grainB, dice, 0, -1, -1)
	lakesFrac.fracInit(iW, iH, lake_grain, dice, 0, -1, -1)

	iValleyThreshold = valleysFrac.getHeightFromPercent(valleys)
	iValleyEdgeThresholdA = valleysFrac.getHeightFromPercent(valleyEdgesA)
	iValleyEdgeThresholdB = valleysFrac.getHeightFromPercent(valleyEdgesB)
	iLakesThreshold = lakesFrac.getHeightFromPercent(lakes)
	iValleyThresholdA = terrainFracA.getHeightFromPercent(valleysA)
	iHillsThresholdA = terrainFracA.getHeightFromPercent(hillsA)
	iPeaksThresholdA = terrainFracA.getHeightFromPercent(peaksA)
	iHillsThresholdB = terrainFracB.getHeightFromPercent(hillsB)
	iPeaksThresholdB = terrainFracB.getHeightFromPercent(peaksB)

	# Now the main loop, which will assign the plot types.
	for x in range(iW):
		for y in range(iH):
			i = y*iW + x
			valleyVal = valleysFrac.getHeight(x,y)
			lakeVal = lakesFrac.getHeight(x,y)
			terrainValA = terrainFracA.getHeight(x,y)
			terrainValB = terrainFracB.getHeight(x,y)
			# Valleys
			if valleyVal <= iValleyThreshold:
				plotTypes[i] = PlotTypes.PLOT_LAND
			elif valleyVal <= iValleyEdgeThresholdA:
				plotTypes[i] = PlotTypes.PLOT_HILLS
			elif valleyVal <= iValleyEdgeThresholdB:
				plotTypes[i] = PlotTypes.PLOT_LAND
			# Oceans and Lakes
			elif lakeVal <= iLakesThreshold:
				plotTypes[i] = PlotTypes.PLOT_OCEAN
			# Terrain A
			elif terrainValA <= iValleyThresholdA:
				plotTypes[i] = PlotTypes.PLOT_LAND
			elif terrainValA >= iPeaksThresholdA:
				plotTypes[i] = PlotTypes.PLOT_PEAK
			elif terrainValA >= iHillsThresholdA and terrainValA < iPeaksThresholdA:
				plotTypes[i] = PlotTypes.PLOT_HILLS
			# Terrain B
			elif terrainValB >= iPeaksThresholdB:
				plotTypes[i] = PlotTypes.PLOT_PEAK
			elif terrainValB >= iHillsThresholdB and terrainValB < iPeaksThresholdB:
				plotTypes[i] = PlotTypes.PLOT_HILLS
			else:
				plotTypes[i] = PlotTypes.PLOT_LAND
...
class HighlandsTerrainGenerator(CvMapGeneratorUtil.TerrainGenerator):
	def __init__(self, fracXExp=-1, fracYExp=-1):
		...
		self.iColdTBottomPercent = 30
		self.iCoolTBottomPercent = 90
		self.iCoolPBottomPercent = 60
		self.iTempDBottomPercent = 90
		self.iTempPBottomPercent = 70
		self.iHotDBottomPercent = 70
		self.iHotPBottomPercent = 60

		self.fColdLatitude = 0.9
		self.fCoolLatitude = 0.8
...
def assignStartingPlots():
	...
	# Also prioritize the positioning of human players according to game options
	shuffledPlayers = []
	for playerLoopTwo in range(iPlayers):
		iRandomIndex = dice.get(len(player_list), "Shuffling Players - Highlands PYTHON")
		iPlayerID = player_list[iRandomIndex]
		if (map.getCustomMapOption(5) == 1 and gc.getPlayer(iPlayerID).isHuman()):
			shuffledPlayers.insert(0, iPlayerID)
		else:
			shuffledPlayers.append(iPlayerID)
		del player_list[iRandomIndex]
