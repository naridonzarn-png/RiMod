...
class CvCustomizableDomesticAdvisor:
	...
	def __init__(self):
		...
		self.COLUMNS_LIST = [
				...
				("AUTOMATION",				80,		"text",	None,					None,					0,									self.calculateAutomation,				None,						"u\"AUTO-EMPHASIZE\""),
				...
				("EPIDEMIC",				30,		"int",	CyCity.getEpidemicProbabilityPercent,	None,	0,									None,									None,						"self.epidemicIcon + u\"E\""),
...
# BUG - Production Grouping - end
	...
	def createDictionaries(self):
		...
		#DERTUEK : New => epidemicIcon
		self.epidemicIcon = u"%c" % CyGame().getSymbolID(FontSymbols.UNHEALTHY_CHAR)
...
#		extraBldgColumns = []
		# DERTUEK : New temporary list => list_to_sort
		list_to_sort = []
		for i in range(gc.getNumBuildingInfos()):
			...
			list_to_sort.append((desc,key,len(icon)))
...
#			extraBldgColumns.append(("BLDG_" + key, 22, "bldg", None, None, 0, self.calculateBuilding, i, "u\"%s\"" % desc))
		...
		for item in list_to_sort:
			self.COLUMNS_LIST.append((item[1], 50 + 15 * item[2], "text", None, None, 0, self.calculateValue, None, u"u\"" + item[0] + u"\""))
...
#		self.COLUMNS_LIST += extraBldgColumns
		...
		for i in range(gc.getNumBuildingClassInfos()):
			...
			desc = info.getDescription()
			list_to_sort.append((desc,key,i))

		list_to_sort.sort()

		for item in list_to_sort:
			self.COLUMNS_LIST.append((item[1], 22, "bldgclass", None, None, 0, self.calculateBuildingClass, item[2], "u\"%s\"" % item[0]))
		...
		for item in list_to_sort:
			self.COLUMNS_LIST.append((item[1], 22, "bldg", None, None, 0, self.calculateBuilding, item[2], "u\"%s\"" % item[0]))
		...
		if self.SPECIALIST_ICON_DICT == None:
			...
			self.SPECIALIST_ICON_DICT = {
				0 : self.fistIcon, # Citizen
				1 : self.militaryIcon, # Craftsman
				2 : self.religionIcon, # Priest
				3 : self.cultureIcon, # Artist
				4 : self.researchIcon, # Scientist
				5 : self.goldIcon, # Merchant
				6 : self.hammerIcon, # Engineer
				7 : self.espionageIcon, # Spy
				}
...
# BUG - Colony Split - end
		...
		self.nCultureLevelTextOffset = 150
...
#					szEffects += u"%d%s " % (iEffect, self.commerceIcons[eCommerceType])
	...
	def drawContents (self, page):
		...
		if(self.customizing):
			...
			screen.setTableColumnHeader (self.CUSTOMIZE_PAGE, 1, u"<font=2>NAME</font>", 320 )
			...
			if(screen.getTableNumRows(self.COLUMNS_LIST_PAGE) != len(self.COLUMNS_LIST)):
				...
				screen.setTableColumnHeader (self.COLUMNS_LIST_PAGE, 1, u"<font=2>NAME</font>", 320 )
				screen.setTableColumnHeader (self.COLUMNS_LIST_PAGE, 2, u"<font=2>TITLE</font>", 200 )
...
# BUG - Colony Split - end
	...
	def save(self, inputClass):
		...
		# DERTUEK
		if (not name):
			# is subdirectory "CustomDomAdv" exists in Settings directory ?
			subdir = BugPath.join(BugPath.getSettingsDir(), "CustomDomAdv")
			if not BugPath.isdir(subdir):
				# NO => create it
				try:
					BugPath.safeInfoPath("BugPath - creating '%s'", subdir)
					os.makedirs(subdir)
				except OSError:
					BugUtil.trace("Cannot create directory 'CustomDomAdv' in '%s", BugPath.getSettingsDir())

			if BugPath.isdir(subdir):
				# subdirectory exists => It's ok to create the file
				name = BugPath.join(subdir, "CustomDomAdv.txt")
				...
				self.PAGES =	[
					...
					},
					{
						...
						"columns" : [
							...
							("RELIGIONS", 170, "text"),
							("AUTOMATION", 100, "text"),
							("SPECIALISTS", 160, "text"),
							("HAPPY", 30, "int"),
							("GROWTH", 35, "int"),
							("FOOD", 35, "int"),
							("PRODUCTION", 50, "int"),
							("BASE_COMMERCE", 50, "int"),
							("GOLD", 50, "int"),
							("RESEARCH", 50, "int"),
							("CULTURE_RATE", 50, "int"),
							("CULTURE", 63, "int"),
							("GREATPEOPLE_RATE", 50, "int"),
							("GREATPEOPLE", 55, "int"),
							("GREATPEOPLE_TURNS", 47, "int"),
						]
					},
					{
