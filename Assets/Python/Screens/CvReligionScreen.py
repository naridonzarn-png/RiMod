...
#		self.DX_RELIGION = 98
		self.X_RELIGION_START = 160
		self.DX_RELIGION = 82
# MOD - END - More Religions
		self.Y_RELIGION = 35
		self.Y_FOUNDED = 90
...
#		self.X_RELIGION_AREA = 45
		self.X_RELIGION_AREA = 25
# MOD - END - More Religions
		self.Y_RELIGION_AREA = 84
# MOD - START - More Religions
#		self.W_RELIGION_AREA = 934
		self.W_RELIGION_AREA = 974
# MOD - END - More Religions
...
#		self.X_CITY2_AREA = 522
		self.X_CITY1_AREA = 25
		self.X_CITY2_AREA = 562
# MOD - START - More Religions
		self.Y_CITY_AREA = 282
# MOD - START - More Religions
#		self.W_CITY_AREA = 457
		self.W_CITY_AREA = 497
# MOD - END - More Religions
...
# MOD - END - More Religions
	...
	def drawReligionInfo(self):
		...
		for iRel in self.RELIGIONS:
			...
			szLabel = u"<font=1>" + gc.getReligionInfo(iRel).getDescription() + u"</font>"
...
#				szLabel = localText.changeTextColor(szLabel, gc.getInfoTypeForString("COLOR_YELLOW"))
		...
		szLabel = u"<font=1>" + localText.getText("TXT_KEY_RELIGION_SCREEN_NO_STATE", ()) + u"</font>"
#		if (self.iReligionSelected == gc.getNumReligionInfos()):
#			szLabel = localText.changeTextColor(szLabel, gc.getInfoTypeForString("COLOR_YELLOW"))
		screen.setLabelAt(szName, szArea, szLabel, CvUtil.FONT_CENTER_JUSTIFY,  self.X_SCROLLABLE_RELIGION_AREA + xLoop, self.Y_RELIGION_NAME, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)		
	...
	def drawHelpInfo(self):
		...
		# Founded...
		screen.setLabelAt("", szArea, u"<font=1>" + localText.getText("TXT_KEY_RELIGION_SCREEN_DATE_FOUNDED", ()) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, self.LEFT_EDGE_TEXT, self.Y_FOUNDED, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)

		# Date Founded:
		xLoop = self.X_RELIGION_START
		for iRel in self.RELIGIONS:
			if (gc.getGame().getReligionGameTurnFounded(iRel) >= 0):
				szFounded = u"<font=1>" + CyGameTextMgr().getTimeStr(gc.getGame().getReligionGameTurnFounded(iRel), false) + u"</font>"
...
#		screen.setLabelAt("", szArea, "", CvUtil.FONT_CENTER_JUSTIFY, xLoop, self.Y_FOUNDED, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
		...
		# Holy City...
		screen.setLabelAt("", szArea, u"<font=1>" + localText.getText("TXT_KEY_RELIGION_SCREEN_HOLY_CITY", ()) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, self.LEFT_EDGE_TEXT, self.Y_HOLY_CITY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
		...
		for iRel in self.RELIGIONS:
			if (gc.getGame().getReligionGameTurnFounded(iRel) >= 0):
				...
				if pHolyCity.isNone():
					szFounded = u"<font=1>" + localText.getText("TXT_KEY_NONE", ()) + u"</font>"
					...
					szFounded = u"<font=1>" + localText.getText("TXT_KEY_UNKNOWN", ()) + u"</font>"
					screen.setLabelAt("", szArea, szFounded, CvUtil.FONT_CENTER_JUSTIFY, xLoop, self.Y_HOLY_CITY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
				else:
					szFounded = u"<font=1>" + pHolyCity.getName() + u"</font>"
					screen.setLabelAt("", szArea, u"<font=1>(%s)" % gc.getPlayer(pHolyCity.getOwner()).getCivilizationAdjective(0)+ u"</font>", CvUtil.FONT_CENTER_JUSTIFY, xLoop, self.Y_HOLY_CITY+8, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
...
#				screen.setLabelAt("", szArea, szFounded, CvUtil.FONT_CENTER_JUSTIFY, xLoop, self.Y_HOLY_CITY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
		...
		# Influence...
		screen.setLabelAt("", szArea, u"<font=1>" + localText.getText("TXT_KEY_RELIGION_SCREEN_INFLUENCE", ()) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, self.LEFT_EDGE_TEXT, self.Y_INFLUENCE, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)

		xLoop = self.X_RELIGION_START
		for iRel in self.RELIGIONS:
			if (gc.getGame().getReligionGameTurnFounded(iRel) >= 0):
				szFounded = u"<font=1>" + str(gc.getGame().calculateReligionPercent(iRel)) + "%" + u"</font>"
...
# BUG - start
			...
			screen.setLabelAt("", szArea, self.szCities, CvUtil.FONT_LEFT_JUSTIFY, self.LEFT_EDGE_TEXT, iY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)

			xLoop = self.X_RELIGION_START
			for iRel in self.RELIGIONS:
				if (gc.getGame().getReligionGameTurnFounded(iRel) >= 0):
					szFounded = u"<font=1>" + "%i" % (iCities[iRel]) + u"</font>"
			...
			for iRel in self.RELIGIONS:
				if (gc.getGame().getReligionGameTurnFounded(iRel) >= 0):
					szFounded = u"<font=1>" + "%i" % (iTemple[iRel]) + u"</font>"
					screen.setLabelAt("", szArea, szFounded, CvUtil.FONT_CENTER_JUSTIFY, xLoop, iY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
				xLoop += self.DX_RELIGION
			...
			for iRel in self.RELIGIONS:
				if (gc.getGame().getReligionGameTurnFounded(iRel) >= 0):
					szFounded = u"<font=1>" + "%i" % (iMonastery[iRel]) + u"</font>"
					screen.setLabelAt("", szArea, szFounded, CvUtil.FONT_CENTER_JUSTIFY, xLoop, iY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
				xLoop += self.DX_RELIGION
			...
			for iRel in self.RELIGIONS:
				if (gc.getGame().getReligionGameTurnFounded(iRel) >= 0):
					szFounded = u"<font=1>" + "%i [%i]" % (iMissionaries_Active[iRel], iMissionaries_Construct[iRel]) + u"</font>"
					screen.setLabelAt("", szArea, szFounded, CvUtil.FONT_CENTER_JUSTIFY, xLoop, iY, self.DZ, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
				xLoop += self.DX_RELIGION
	...
	def BUGConstants(self):
		...
		self.szCities = u"<font=1>" + localText.getText("TXT_KEY_BUG_RELIGIOUS_CITY", ()) + u"</font>"
		self.szTemples = u"<font=1>" + localText.getText("TXT_KEY_BUG_RELIGIOUS_TEMPLE", ()) + u"</font>"
		self.szMonastaries = u"<font=1>" + localText.getText("TXT_KEY_BUG_RELIGIOUS_MONASTARY", ()) + u"</font>"
		self.szMissionaries = u"<font=1>" + localText.getText("TXT_KEY_BUG_RELIGIOUS_MISSIONARY", ()) + u"</font>"
...
# start of BUG indent for new code
		if AdvisorOpt.isReligious():
			...
			screen.setTableColumnHeader(self.TABLE_ID, self.COL_CITY_NAME, u"<font=1>" + self.sCity + u"</font>", 115)

			for iRel in range(self.NUM_RELIGIONS):   # columns for religious icons
				if (gc.getGame().getReligionGameTurnFounded(iRel) >= 0):
					szReligionIcon = u"<font=1>%c</font>" %(gc.getReligionInfo(iRel).getChar())
			...
			for type in ReligionUtil.getUnitTypes():
				screen.setTableColumnHeader(self.TABLE_ID, self.COL_FIRST_UNIT + type.index, u"<font=1>%s</font>" % type.icon, 30)

			# columns for buildings (temples, monasteries, cathedral, shrine)
			for type in ReligionUtil.getBuildingTypes():
				screen.setTableColumnHeader(self.TABLE_ID, self.COL_FIRST_BUILDING + type.index, u"<font=1>%s</font>" % type.icon, 30)

			# column for religious impact
			screen.setTableColumnHeader(self.TABLE_ID, self.COL_EFFECTS, "", 430)
			...
			for iCity in range(len(cityList)):
				...
				screen.setTableText(self.TABLE_ID, self.COL_CITY_NAME, iCity, u"<font=1>" + pLoopCity.getName() + u"</font>", "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
				...
				for iRel in range(self.NUM_RELIGIONS):
					if (gc.getGame().getReligionGameTurnFounded(iRel) >= 0):
						...
						if iRel in lHolyCity:
							szReligionIcon = u"<font=1>%c</font>" %(gc.getReligionInfo(iRel).getHolyCityChar())
