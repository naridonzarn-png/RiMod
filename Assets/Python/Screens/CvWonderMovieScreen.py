...
class CvWonderMovieScreen:
	...
	def interfaceScreen (self, iMovieItem, iCityId, iMovieType):
		...
		# MOD - START - Static Wonder Images
		if not gc.findAssetFile(self.szMovieFile):
		  if self.szMovieFile.endswith(".bik"):
		  	self.szMovieFile = self.szMovieFile.replace(".bik", ".dds")
		# MOD - END - Static Wonder Images
