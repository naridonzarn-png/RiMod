...
# BUG - Stack Promotions - end
	...
	def displayHelpHover(self, sKey):
		sText = u"<font=1>%s</font>" % BugUtil.getPlainText(sKey)
	...
	def getMaxCol(self):
		...
		elif (self.xResolution < 1440):
			return ((self.xResolution - (iMultiListXL+iMultiListXR) - 128) / 34)
		else:
			return ((self.xResolution - (iMultiListXL+iMultiListXR) - 408) / 34)
		
	def getMaxRow(self):
		return ((self.yResolution - 160) / max(PleOpt.getVerticalSpacing(),1)) 
...
#Adjust the horizontal position
	def getX(self, nCol):
		if (self.xResolution == 1024):
			return 365 + (nCol * PleOpt.getHoriztonalSpacing())
		else:	
			return 465 + (nCol * PleOpt.getHoriztonalSpacing())		
#Adjust the vertical position of units		
	def getY(self, nRow):
...
#Adjust of inicial position of small buttons
		nYOff = 130 + 4
		if (self.xResolution == 1024):		
			nXOff = 336 - 12
		else:	
		...
		nSize	= 24
		nDist	= 22
#Adjust the gap between the small buttons
		nGap    = 4
		nNum	= 0
	...
	def showPromoInfoPane(self, id):
		...
		# promo info
		szPromoInfo = u"<font=1>" + mt.removeLinks(CyGameTextMgr().getPromotionHelp(iPromo, false)) + u"</font>\n"
		...
		if iMaxLevel <> iLevel:
			...
			szLevel = u"<font=1>" + localText.getText("INTERFACE_PANE_LEVEL", ()) + u"%i / %i" % (iLevel, (iMaxLevel+iLevel)) + u"</font>\n"
		else:
			# actual 
			szLevel = u"<font=1>" + localText.getText("INTERFACE_PANE_LEVEL", ()) + u"%i" % iLevel + u"</font>\n"

		# unit experience (actual / needed)
		iExperience = pUnit.getExperience()
		if (iExperience > 0):
			szExperience = u"<font=1>" + localText.getText("INTERFACE_PANE_EXPERIENCE", ()) + u": %i / %i" %(iExperience, pUnit.experienceNeeded()) + u"</font>\n"
		else:
			szExperience = u""
	...
	def showUnitInfoPane(self, id):
		...
		if (pUnit.getOwner() != gc.getGame().getActivePlayer()):
			...
			szOwner = u"<font=1> [" + localText.changeTextColor(pOwner.getName(), pOwner.getPlayerColor()) + u"]</font>"
		else:
			szOwner = u""

		# unit type description + unit name (if given)
		szUnitName = u"<font=1>" + localText.changeTextColor(pUnit.getName(), PleOpt.getUnitNameColor()) + szOwner + u"</font>\n"
		...
		if fCurrStrength != fMaxStrength:
			...
			szMaxStrength 		= u""		
			szTurnsToHeal 		= u""
		szStrength = u"<font=1>" + szCurrStrength + szMaxStrength + szTurnsToHeal + u"%c" % CyGame().getSymbolID(FontSymbols.STRENGTH_CHAR) + u"</font>\n" 
		...
		if ( pUnit.movesLeft() != iMaxMoves * gc.getMOVE_DENOMINATOR() ):
			...
			szMaxMoves 	= u""
		szMovement = u"<font=1>" + szCurrMoves + szMaxMoves + u"%c"%(CyGame().getSymbolID(FontSymbols.MOVES_CHAR)) + szAirRange + u"</font>\n"

		# compressed display for standard display
		szStrengthMovement = u"<font=1>" + szCurrStrength + szMaxStrength + szTurnsToHeal + u"%c" % CyGame().getSymbolID(FontSymbols.STRENGTH_CHAR) + ", " + \
	...
	#				break
	#		szCiv = u"<font=1>100% " + localText.changeTextColor(pLoopPlayer.getCivilizationAdjective(0), pLoopPlayer.getPlayerColor()) + u"</font>\n"
		...
		if iCargoSpace > 0:
			...
			szCargo = u"<font=1>" + localText.getText("TXT_KEY_UNIT_HELP_CARGO_SPACE", (iCargo, iCargoSpace ) ) + u"</font>\n"
		else:
			szCargo = u""
		...
		if iFortifyBonus > 0:
			szFortifyBonus = u"<font=1>" + localText.getText("TXT_KEY_UNIT_HELP_FORTIFY_BONUS", (iFortifyBonus, )) + u"\n" + u"</font>"
		...
		if szEspionage:
			szEspionage = u"<font=1>" + szEspionage + u"\n</font>"
		
		# unit type specialities 
		szSpecialText 	= u"<font=1>" + localText.getText("TXT_KEY_PEDIA_SPECIAL_ABILITIES", ()) + u":\n" + CyGameTextMgr().getUnitHelp( iUnitType, true, false, false, None )[1:] + u"</font>"
