		...
		for j in range( gc.getNumUnitClassInfos() ):
			...
			if (eLoopUnit != -1):
				if (isTechRequiredForUnit(self.iTech, eLoopUnit)):
	        			screen.attachImageButton( panelName, "", gc.getUnitInfo(eLoopUnit).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM,
...
#---Eingefügt START - kann komplett gelöscht werden --------------
		...
		kCivilization = gc.getCivilizationInfo(CyGame().getActiveCivilizationType())
		# MOD - END - Unique Improvements
		for j in range(gc.getNumBuildInfos()):
			# MOD - START - Unique Improvements
			if (kCivilization.canBuild(j)):
			# MOD - END - Unique Improvements
				bTechFound = 0;
		...
		for j in range(gc.getNumBonusInfos()):
			if (gc.getBonusInfo(j).getTechReveal() == self.iTech):
				...
				screen.attachImageButton( panelName3, "", sButton, GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, j, 1, False )
